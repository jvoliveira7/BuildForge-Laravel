...
<div class="container mx-auto text-white py-8">
    <h1 class="text-3xl font-bold mb-6">Editar Produto: {{ $produto->nome }}</h1>
    ...
    <form action="{{ route('admin.produtos.update', $produto->id) }}" method="POST" enctype="multipart/form-data" class="max-w-lg">
        @csrf
        @method('PUT')
        ...
        <textarea name="descricao" id="descricao" rows="4"
            class="w-full px-3 py-2 mb-4 rounded text-black">{{ old('descricao', $produto->descricao) }}</textarea>
        ...
        <label class="block mb-2 font-semibold" for="imagem">Imagem Atual</label>
        @if ($produto->imagem)
            <img src="{{ asset('storage/' . $produto->imagem) }}" alt="{{ $produto->nome }}" class="mb-4 max-h-40 rounded">
        @else
            <p class="mb-4 text-gray-400">Nenhuma imagem cadastrada.</p>
        @endif

        <label class="block mb-2 font-semibold" for="imagem">Trocar Imagem</label>
